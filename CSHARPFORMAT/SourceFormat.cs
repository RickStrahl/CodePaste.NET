...
namespace Manoli.Utils.CSharpFormat
{
	...
	public abstract class SourceFormat
	{
        ...
        /// <summary>
        /// Determines whether code snippets can word wrap
        /// NOT RECOMMENDED! Use only for displaying on mobile
        /// devices etc.
        /// </summary>
        public bool AllowWordWrapping
        {
            get { return _AllowWordWrapping; }
            set { _AllowWordWrapping = value; }
        }
        private bool _AllowWordWrapping = false;
		...
		public string FormatSubCode(string source)
		{
			...
			string result = FormatCode(source, false, false, false, true);
            if (!IsLineBreak)
			...
			bool alternate, bool embedStyleSheet, bool subCode)
		{
            ...
            if (this.AllowWordWrapping)
                // fix up leading spaces with #@@#
                source = this.FixWordWrapping(source); 
        ...
        private string SplitCodeLines(string source, int codeLineWidth)
        {
            if (codeLineWidth < 20)
                return source;

            string[] lines = source.Split(new char[1] { '\n'});
            ...
            foreach (string line in lines)
            {
                ...
                if (codeLine.Length <= codeLineWidth)
                {
                ...
                bool firstLine = true;
                while (codeLine.Length > codeLineWidth)
                {
                    string line50 = codeLine.Substring(0, codeLineWidth);
                    int index = line50.LastIndexOf(' ');
                    if (!firstLine)
                        sb.Append(" ... ");
                    
                    if (index > 0)
                    {                        
                        sb.AppendLine(line50.Substring(0,index));
        ...
        public string FixWordWrapping(string source)
        {
            string[] lines = source.Split(new char[1] { '\n'});
            ...
            foreach (string line in lines)
            {
                ...
                Match match = Regex.Match(codeLine,"^\\s*");
                string leadSpaces = "";
                if (match != null)                
                    leadSpaces = match.Value.Replace(" ","#@@#");
                
                sb.AppendLine(leadSpaces + codeLine);
            }
